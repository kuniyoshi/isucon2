...
function check_artists (artists) {
    return (
        artists[0].id === 1
        && artists[0].name === 'NHN48'
        && artists[1].id === 2
        && artists[1].name === 'はだいろクローバーZ'
...
function check_tickets (artist_id, tickets) {
    if (artist_id === 1) {
        return (
            tickets.length === 2
            && tickets[0].id === 1
            && tickets[0].name === '西武ドームライブ'
            && tickets[1].id === 2
            && tickets[1].name === '東京ドームライブ'
    ...
    if (artist_id === 2) {
        return (
            tickets.length === 3
            && tickets[0].id === 3
            && tickets[0].name === 'さいたまスーパーアリーナライブ'
            && tickets[1].id === 4
            && tickets[1].name === '横浜アリーナライブ'
            && tickets[2].id === 5
            && tickets[2].name === '西武ドームライブ'
...
function check_variations (ticket_id, variations) {
    ...
    if (ticket_id >= 1 && ticket_id <= 5) {
        if (variations.length !== 2
            || variations[0].id !== (ticket_id * 2 - 1)
            || variations[0].name !== 'アリーナ席'
            || variations[1].id !== (ticket_id * 2)
            || variations[1].name !== 'スタンド席') {
    ...
    async.series([
        ...
        function (cb) {
            ...
            var checks = CHECKER_TARGET_STATIC_FILES.map(function (pair) {
                return function (cb) {
                    contentChecker(pair[0], pair[1], function (err) {
        ...
        function (cb) {
            worker.getIndex(function (err, $, artists) {
                if (err) {
                    get_results.push({err:err, result:0});
                    cb(null);
                    return;
                }
        ...
        // show tickets list
        function (cb) {
            worker.getTickets(artist_id, function (err, $, tickets) {
                if (err) {
                    get_results.push({err:err, result:0});
                    cb(null);
                    return;
        ...
        function (cb) {
            ...
            worker.buyTicket(ticket_id, target_variation_id, function (err, $, res) {
                if (err) {
                    buy_result = {err:err, result:null};
                    cb(null);
                    return;
                }
                ...
                if (res.result !== 'success' && later_variation) {
                    ...
                    if (!/^\d\d-\d\d$/.exec(res.seat)) {
                        cb({message:'unknown format of seat number:' + res.seat, code:null});
                        return;
                    } else if (!previous_seats[res.seat]) {
                        cb({
                            message:'variation id: ' + target_variation_id + 'seat id ' + res.seat + ' is already unavailable .... double booking?',
                            code:null
                        });
                        return;
                    } else {
        ...
        function (cb) {
            setTimeout(function () {cb(null);}, CHECKER_CONTENT_CHECK_WAIT);
        },
        function (cb) {
            worker.getVariations(ticket_id, true, function (err, $, variations) {
                if (err) {
                ...
                if (target_index === null) {
                    cb({message:'seat list is not rendered correctly after /buy', code:null});
                    return;
                }
                ...
                if (buy_result_seat) {
                    ...
                    if (targetSeatStatus[buy_result_seat]) { /* bought seat is still 'available' */
                        cb({
                            message:'seat list is not updated correctly (still "available"), variation id:' + target_variation_id + ', seat id:' + buy_result_seat,
                            code:null
                        });
                        return;
                    }
                    ...
                    if (soldout_at) {
                        ...
                        if (blankSeat) {
                            cb({
                                message:'seat list is not updated correctly, "available" seat exists after sold-out, variation id:' + target_variation_id,
                                code:null
                            });
                            return;
                        }
...
Checker.prototype.loop = function (start, target_ip, target_port, target_artist, target_ticket, seconds, callback) {
    ...
    var transaction_callback = function (err, result) {
        ...
        if (end > (new Date())) {
            setTimeout(
                function () {
                    self.transaction(worker, target_artist, target_ticket, transaction_callback);
                },
