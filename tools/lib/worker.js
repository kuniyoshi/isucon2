...
function execute_request_once (options, timeout_milliseconds, callback) {
    ...
    var req = http.request(options, function (res) {
        if (timeouted) {
            return;
        } /* this request is already timeouted */
        if (res.statusCode !== 200) {
            callback({message:'Response code: ', code:res.statusCode});
            return;
        }
        var chunks = [];
        var chunkSize = 0;
        res.on('data', function (chunk) {
            ...
            if (res.headers['content-encoding'] && res.headers['content-encoding'] === 'gzip') {
                zlib.gunzip(buf, function (error, result) {
                    if (error) {
                        console.error(error);
                        callback({message:'Failed to gunzip of content body:' + error.message});
                        return;
                    }
...
function execute_request (options, timeout_milliseconds, retries, callback) {
    ...
    execute_request_once(options, timeout_milliseconds, function (err, content) {
        ...
        if (err && retries > 0) {
            execute_request(options, timeout_milliseconds, retries - 1, callback);
            return;
        }
        callback(err, content);
    });
...
Worker.prototype.checkCsv = function (bought, callback) {
    ...
    execute_request(options, this.timeout.post, retries, function (err, content) {
        ...
        if (failures.length > 0) {
            callback({message:'order record not found in csv, for:' + failures.map(function(r){return r.seat + '/' + r.memberId;}).join(','), code:null});
            return;
        }
        callback(null);
    });
