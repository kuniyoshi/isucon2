...
var HTTP_LOAD_TARGETS = [
    ['path', '/'],
    ['path', '/css/ui-lightness/jquery-ui-1.8.24.custom.css'],
    ['path', '/css/isucon2.css'],
    ['path', '/js/jquery-1.8.2.min.js'],
    ['path', '/js/jquery-ui-1.8.24.custom.min.js'],
    ['path', '/js/isucon2.js'],
...
function createUrlFile(filePath, target_ip, target_port, target_artist, target_ticket) {
    var genURL = function (path) {
        return 'http://' + target_ip + (target_port === 80 ? '' : ':' + target_port) + path;
    };
    ...
    var child = child_process.exec(command, function (err, stdout, stderr) {
        ...
        if (stdout) {
            ...
            if (result.fetches < 1 || !result.status[200] || result.status[200] < 1) {
                callback({message:'GET success fetches are too few',code:null}, result);
                return;
            }
        }
