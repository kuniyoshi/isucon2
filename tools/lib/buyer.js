...
Buyer.prototype.transaction = function (worker, artist_id, ticket_id, callback) {
    ...
    async.series([
        ...
        function (cb) {
            worker.getVariations(ticket_id, false, function (err, $, variations) {
                if (err) {
                    get_results.push({err:err, result:0});
                    cb(null);
                    return;
                ...
                if (variations.length < 1) {
                    get_results.push({err:{message:'No one tickets are rendered in /ticket/x'}, result:0});
                    cb(null);
                    return;
                }
