...
post '/buy' => sub {
    ...
    $self->dbh->query( 'INSERT INTO order_request (member_id) VALUES (?)',
        $member_id, );
...
'SELECT order_request.*, stock.seat_id, stock.variation_id, stock.updated_at
    ...
    for my $order ( @$orders ) {
        $body .= join ',',
          @{$order}{qw( id member_id seat_id variation_id updated_at )};
        $body .= "\n";
    }
    $c->res->body( $body );
    $c->res;
...
post '/admin' => sub {
    ...
    open( my $fh, '<',
        $self->root_dir . '/../config/database/initial_data.sql' )
      or die $!;
    for my $sql ( <$fh> ) {
        chomp $sql;
